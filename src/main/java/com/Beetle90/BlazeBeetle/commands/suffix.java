...
public class suffix extends CommandBase {
	...
	public String getCommandUsage(ICommandSender sender) {
		return "Applies a suffix to the targeted player with the option to use color. /suffix player_name suffix_text color";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (sender instanceof EntityPlayer){
			...
			if (args.length == 0 || args.length == 1){
				player.addChatMessage(new TextComponentString(TextFormatting.RED + "Error: Please use /suffix <player name> <suffix_text> optional:<color>"));
			}
				...
				if (affectedP != null){
					...
					if (args[1] != null && !args[1].equalsIgnoreCase("clear")){
						namestuff.setSuffix(args[1].toString());
					}
					else if (args[1].equalsIgnoreCase("clear")){
						namestuff.setSuffix("empty");
					}
				}
			}
				...
				if (affectedP != null){
					Inamestuff namestuff = affectedP.getCapability(namestuffProvider.NAMESTUFF_CAP,null);
					namestuff.setSuffix(args[1].toString(),args[2].toString()); //MAY NOT WORK WITH ARGS[2] NO CHECK ON 3rd ARGUMENT.
				}
			}
		}
	}
